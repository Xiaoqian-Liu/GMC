...
void mmprod(matrix A, matrix B, matrix C);

/* Matrix-Matrix multiplication, C=A^TB */
void mtmprod(matrix A, matrix B, matrix C);
...
/* Soft-thresholding with group structure */
/*
...
out - the solution from the soft-threshloding (output)
*/
void soft_group(double *x, int *p, double *lambda, int *group, double *K, double *out);
...
/*
For the FB updates in group GMC

*/

void FB(double *y, matrix X, matrix Z, matrix XX, double *beta0, double *v0, double lambda, 
        int *group, double *K, double t, int *max_iters, double *tol, 
        double *beta, double *v, int *iters);
...
void mmprod(matrix A, matrix B, matrix C)
{
  ...
  dgemm_(&TRANSA, &TRANSB, &m, &n, &k, &alpha, A.data, &m, B.data, &k, &beta, C.data, &m);
}


void test_mmprod(double *a, double *b, double *c, int *m, int *k, int *n)
{
  matrix A;
  A.Nrow = *m;
  A.Ncol = *k;
...
void mtmprod(matrix A, matrix B, matrix C)
{
  ...
  dgemm_(&TRANSA, &TRANSB, &m, &n, &k, &alpha, A.data, &k, B.data, &k, &beta, C.data, &m);
}


void test_mtmprod(double *a, double *b, double *c, int *m, int *k, int *n)
{
  matrix A;
  A.Nrow = *k;
  A.Ncol = *m;
    ...
    for (iter = 0; iter < *maxiters; iter++)
    {
       ...
       //======================= compute and store residuals =====================
        ...
        if((stop && iter >= 5) || iter >= *maxiters-1){
            ...
            *Num_of_iters = iter+1;   // outs$iters = iter, iter  starts from 0, check all iters!!!!!!
            break;
        }
        ...
        double *beta, double *v, int *iters)
{
    ...
    mtvprod(X, y, Xty); // Xty= t(X) %*% y

    // ============ Some itermediate values used in the iterations
    double *betav = (double *)calloc(p, sizeof(double));
    double *dbeta = (double *)calloc(p, sizeof(double));
    double *dv = (double *)calloc(p, sizeof(double));
    double *Xb = (double *)calloc(n, sizeof(double));
    double *XXb = (double *)calloc(p, sizeof(double));

    double *bb = (double *)calloc(p, sizeof(double));
    double *vv = (double *)calloc(p, sizeof(double));

    double *bdiff = (double *)calloc(p, sizeof(double));
    double *vdiff = (double *)calloc(p, sizeof(double));

    double param, bdiff_norm, vdiff_norm, b_norm, v_norm, cd1, cd2;
    param = t*lambda;
    ...
    for (k = 0; k < *max_iters; k++){
       ...
       if (n > p){
          ...
          for (int j = 0; j < p; j++){
            betav[j] = (beta0[j]-v0[j])/n; //  x0 = x
          }
          // compute dv
          mvprod(Z, betav, dv); // dv = Z bv/n
          // compute dbeta
          mvprod(XX, beta0, XXb);  // XXb = XX%*%beta_last

          for (int j = 0; j < p; j++){
            dbeta[j] = (XXb[j]-Xty[j])/n - dv[j]; // dbeta = (Xb - Xty)/n - dv
          }   

        }else{
           // compute Xb Here it's X%*%beta
           mvprod(X, beta0, Xb);  // Xb = X beta_last
           // compute the  difference
           for (int j = 0; j < p; j++){
             betav[j] = (beta0[j]-v0[j])/n; //  x0 = x
           }
        ...
        // get beta and v in the soft-threshold
        for (int j = 0; j < p; j++){
             bb[j] = beta0[j]- t*dbeta[j]; // beta_last - t*dbeta
             vv[j] = v0[j] + t*dv[j]; // v_last + t*dv
        }
        // update beta and v 
        soft_group(bb, &p, &param, group, K, beta);
        soft_group(vv, &p, &param, group, K, v);
        // check convergence
        for (int j = 0; j < p; j++){
             bdiff[j] = beta[j]- beta0[j]; // beta_last - t*dbeta
             vdiff[j] = v[j] - v0[j]; // v_last + t*dv
        }
        vec_norm(bdiff, &p, &bdiff_norm);
        vec_norm(beta0, &p, &b_norm);
        vec_norm(vdiff, &p, &vdiff_norm);
        vec_norm(v0, &p, &v_norm);
        cd1 = bdiff_norm/b_norm;
        cd2 = vdiff_norm/v_norm;
        if (cd1 < *tol && cd2 < *tol){
          break;
        }else{
          // update for the next iterate
           for (int j = 0; j < p; j++){
             beta0[j] = beta[j]; // beta_last - t*dbeta
             v0[j] = v[j]; // v_last + t*dv
           } 
        }
...
void FB_call(double *y, int *n, int *p, double *X_data, double *Z_data, double *XX_data,
             double *beta0, double *v0, double *lambda, int *group, double *K, double *t, 
             int *max_iters, double *tol, double *beta, double *v, int *iters)
{
    ...
    FB(y, X, Z, XX, beta0, v0, *lambda, group, K, *t, max_iters, tol, beta, v, iters);
}
